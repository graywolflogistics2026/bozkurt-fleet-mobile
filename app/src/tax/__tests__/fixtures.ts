...
// that row was seeded. The CA `bracket` table below is a TEST-ONLY fixture:
// the live row currently holds just a placeholder string for CA pending
// verification (see docs/ADMIN_RUNBOOK.md — "no copy of them outside the
// database ... should be treated as authoritative"), so this fixture exists
// purely to exercise the bracket-table code path deterministically. MA's
// `flat` rate (5%) is its real, stable, long-published rate — used here so
// the flat_adjustments surtax path has something real to layer on top of,
// per ADMIN_RUNBOOK's note that MA's own bare rate isn't itemized yet.
