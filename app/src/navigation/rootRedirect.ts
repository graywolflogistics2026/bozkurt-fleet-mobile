...
export function resolveRootRedirect({
  ...
  if (hasSession && !needsTos && needsOnboarding && !onOnboardingScreen) return '/onboarding';
  if (hasSession && !needsTos && !needsOnboarding && (inAuthGroup || onTosScreen || onOnboardingScreen || onIntroScreen)) {
    return '/(tabs)';
  }
  return null;
}
