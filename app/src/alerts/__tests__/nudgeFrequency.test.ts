import { selectNudgesToShow, recordNudgesShown, silenceNudgeTopic, unsilenceNudgeTopic, MAX_NUDGES_PER_DAY } from '@/src/alerts/nudgeFrequency';
import type { NudgeCandidate } from '@/src/alerts/missingDataNudges';
...
describe('selectNudgesToShow', () => {
  ...
  test('a topic shown 3 days ago is still within the once-per-week cap — excluded', () => {
    const state = { needsReviewReceipts: { lastShownAt: new Date(NOW.getTime() - 3 * 24 * 60 * 60 * 1000).toISOString(), silencedAt: null } };
    expect(selectNudgesToShow([candidate('needsReviewReceipts')], state, null, NOW)).toEqual([]);
  });

  test('a topic shown 8 days ago is past the weekly cap — included again', () => {
    const state = { needsReviewReceipts: { lastShownAt: new Date(NOW.getTime() - 8 * 24 * 60 * 60 * 1000).toISOString(), silencedAt: null } };
    expect(selectNudgesToShow([candidate('needsReviewReceipts')], state, null, NOW)).toHaveLength(1);
  });

  test('a silenced topic never shows again, no matter how long ago', () => {
    const state = { needsReviewReceipts: { lastShownAt: null, silencedAt: new Date(NOW.getTime() - 30 * 24 * 60 * 60 * 1000).toISOString() } };
    expect(selectNudgesToShow([candidate('needsReviewReceipts')], state, null, NOW)).toEqual([]);
  });
  ...
  test(`never more than ${MAX_NUDGES_PER_DAY} candidates in one call`, () => {
    ...
    expect(selectNudgesToShow(candidates, {}, null, NOW)).toHaveLength(MAX_NUDGES_PER_DAY);
  });
  ...
  test('nudges shown YESTERDAY do not count against today\'s cap', () => {
    const state = { noRecentSettlement: { lastShownAt: new Date(NOW.getTime() - 25 * 60 * 60 * 1000).toISOString(), silencedAt: null } };
    expect(selectNudgesToShow([candidate('needsReviewReceipts')], state, null, NOW)).toHaveLength(1);
  });
});
...
describe('recordNudgesShown / silenceNudgeTopic / unsilenceNudgeTopic', () => {
  test('recordNudgesShown stamps lastShownAt for exactly the given topics, leaving others untouched', () => {
    const state = { needsReviewReceipts: { lastShownAt: '2026-08-01T00:00:00.000Z', silencedAt: null } };
    const next = recordNudgesShown(state, ['noRecentSettlement'], NOW);
    expect(next.noRecentSettlement).toEqual({ lastShownAt: NOW.toISOString(), silencedAt: null });
    expect(next.needsReviewReceipts).toEqual(state.needsReviewReceipts);
  });

  test('recordNudgesShown preserves an existing silencedAt for the same topic', () => {
    const state = { needsReviewReceipts: { lastShownAt: null, silencedAt: '2026-08-01T00:00:00.000Z' } };
    ...
    const next = recordNudgesShown(state, ['needsReviewReceipts'], NOW);
  ...
  test('silenceNudgeTopic sets silencedAt without touching lastShownAt', () => {
    const state = { needsReviewReceipts: { lastShownAt: '2026-08-01T00:00:00.000Z', silencedAt: null } };
    const next = silenceNudgeTopic(state, 'needsReviewReceipts', NOW);
  ...
  test('unsilenceNudgeTopic clears silencedAt only', () => {
    const state = { needsReviewReceipts: { lastShownAt: '2026-08-01T00:00:00.000Z', silencedAt: '2026-08-10T00:00:00.000Z' } };
    const next = unsilenceNudgeTopic(state, 'needsReviewReceipts');
  ...
  test('unsilenceNudgeTopic on a topic with no existing entry is a no-op', () => {
    expect(unsilenceNudgeTopic({}, 'needsReviewReceipts')).toEqual({});
  });
});
